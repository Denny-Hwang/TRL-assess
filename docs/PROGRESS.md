...
| 9 — Release v1.0.0                              | **Released** | local merge `phase/09-release`, merged to `main` | 2026-09-16 | Final audit in `docs/final-audit.md`; version 1.0.0; example exports generated; backlog filed as issues #1–#10; tag `v1.0.0` created at the owner's direction. |
...
| 6   | **Pages is enabled but the deploy job is still rejected.** `actions/configure-pages` and `actions/upload-pages-artifact` now succeed on `main` (runs 10 and 11), so the site exists. The `deploy` job then fails in ~2 seconds with no runner and no logs, which is how GitHub reports a deployment that the `github-pages` environment refused. Two things to check, in order: **(a)** Settings → Pages → Build and deployment → **Source must be "GitHub Actions"**, not "Deploy from a branch" — `actions/deploy-pages` is rejected outright when the site builds from a branch; **(b)** Settings → Environments → `github-pages` → Deployment branches must allow `main`, since the environment may still be pinned to the previous default branch name. | Phase 9   | **Needs user action**     |
